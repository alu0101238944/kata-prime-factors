...
class PrimeFactors():
  def __init__(self, number):
    if number > 1:
      self.number = number
      self.needed_primes = self.__compute_needed_primes()
    else:
      raise ValueError('Numbers smaller than 2 have no prime factors')

  def __compute_needed_primes(self):
    needed_primes = [2]
  ...
  def get_prime_factors(self):
    ...
    prime_factors = []
    i = 0
    while number > 1 and i < len(self.needed_primes):
      current_divisor = self.needed_primes[i]
      if current_divisor > sqrt(self.number):
